...
/* Move the default "Publications" title so it doesn’t sit behind the round logo */
.page__title,
.page-title {
  margin-left: 6.8rem;
}

/* Shift right so content clears the logo (similar to News) */
.pub-shell {
  max-width: 1400px;
  margin: 0 auto 4rem;
  padding: 2rem clamp(1.5rem, 4vw, 3rem);
  padding-left: calc(2rem + 5.6rem);
...
<script>
(function() {
  const STORAGE_KEY = 'found-lang-news'; // shared with News
