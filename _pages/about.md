...
.tl-track{position:relative;display:flex;align-items:stretch;width:max-content;min-height:396px;padding:0 var(--shell-pad)}
.tl-track{position:relative;display:flex;align-items:stretch;width:max-content;min-height:600px;padding:0 var(--shell-pad)}
...
.tl-item{position:relative;flex:0 0 288px;z-index:1}
.tl-item{position:relative;flex:0 0 clamp(250px,27vw,340px);z-index:1}
...
.tl-up .tl-stem{bottom:50%;height:70px}
.tl-down .tl-stem{top:50%;height:70px;background:linear-gradient(0deg,rgba(212,175,55,.6),rgba(212,175,55,.1))}
.tl-card{position:absolute;left:.6rem;right:.6rem;background:rgba(255,255,255,.055);border:1px solid rgba(232,245,240,.14);border-radius:16px;padding:.9rem 1rem 1.05rem;backdrop-filter:blur(10px);box-shadow:0 14px 36px rgba(0,0,0,.3);z-index:2;transition:transform .3s var(--transition-smooth),border-color .3s var(--transition-smooth),background .3s var(--transition-smooth),box-shadow .3s var(--transition-smooth),opacity .5s var(--transition-smooth)}
.tl-up .tl-card{bottom:calc(50% + 70px)}
.tl-down .tl-card{top:calc(50% + 70px)}
...
.copro-ex-chev::before{content:'\25BE';font-size:.8rem;line-height:1;color:var(--primary-green)}
.copro-ex-toggle:hover .copro-ex-chev{background:#fff;border-color:rgba(212,175,55,.6)}
.copro-ex-toggle[aria-expanded="true"] .copro-ex-chev{transform:rotate(180deg)}
.copro-ex-panel{overflow:hidden;max-height:0;opacity:0;transition:max-height .4s var(--transition-smooth),opacity .3s ease,margin-top .3s ease}
.copro-ex-panel.is-open{max-height:800px;opacity:1;margin-top:.75rem}
  ...
  <div class="copro-inner">
    ...
    <div class="copro-grid">
      ...
      <div class="copro-example">
        <button type="button" class="copro-ex-toggle" id="copro-ex-toggle" aria-expanded="true" aria-controls="copro-ex-panel">
        ...
        <div class="copro-ex-panel is-open" id="copro-ex-panel">
...
(function(){
  ...
  function setupCoproDisclosure(){
    var btn=document.getElementById('copro-ex-toggle'),panel=document.getElementById('copro-ex-panel');
    if(!btn||!panel)return;
    function set(open){
    ...
    var wide=true;
    try{wide=!(window.matchMedia&&window.matchMedia('(max-width: 900px)').matches)}catch(e){}
    set(wide);
    btn.addEventListener('click',function(){set(panel.classList.contains('is-open')===false)});
  }
  ...
  function init(){
    ...
    setupTouchZoom();setupScrollReveal();setupCounters();setupCoproDisclosure();
  }
