...
:root{
  ...
  --shell-max:1860px;--shell-pad:clamp(.85rem,2.2vw,2rem);
...
.page,#main,.initial-content,.page__inner-wrap,.page__content,.archive{max-width:100%!important;width:100%!important;float:none!important;margin-left:0!important;margin-right:0!important;padding-left:0!important;padding-right:0!important}
...
.lang-toggle{position:absolute;top:1.4rem;right:calc(var(--shell-pad) + 1.75rem);display:inline-flex;gap:.45rem;z-index:3}
...
.title-inner{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad);position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;gap:1.4rem}
...
.project-subtitle{font-size:clamp(1.15rem,2.5vw,1.6rem);font-weight:400;color:rgba(232,245,240,.85);font-family:var(--font-display);font-style:italic;letter-spacing:.02em;line-height:1.55;max-width:980px;margin:0 auto;text-align:center;text-shadow:0 6px 18px rgba(0,0,0,.22);padding:0 var(--shell-pad)}
...
.hero-content{position:relative;z-index:1;max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
...
.featured-pub-card{display:inline-flex;flex-direction:column;align-items:flex-start;gap:.55rem;width:320px;max-width:100%;padding:1.15rem 1.5rem 1.25rem;border-radius:22px;letter-spacing:normal;line-height:1.6;font-weight:400;background:linear-gradient(135deg,rgba(255,255,255,.96) 0%,rgba(253,248,232,.92) 100%);border:1px solid rgba(212,175,55,.4);box-shadow:var(--shadow-md);text-decoration:none;text-align:left;position:relative;isolation:isolate;overflow:hidden;transition:transform .25s var(--transition-smooth),box-shadow .25s var(--transition-smooth),border-color .25s var(--transition-smooth)}
.featured-pub-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(180deg,var(--gold-accent),rgba(212,175,55,.25))}
.featured-pub-card:hover{transform:translateY(-3px);box-shadow:var(--shadow-lg);border-color:rgba(212,175,55,.7)}
.featured-pub-eyebrow{display:inline-flex;align-items:center;gap:.4rem;font-family:var(--font-mono);font-size:.66rem;font-weight:700;letter-spacing:.14em;line-height:1.5;text-transform:uppercase;color:#8a6d00}
.featured-pub-title{font-family:var(--font-body);font-size:1.05rem;font-weight:750;color:var(--dark-green);line-height:1.55;letter-spacing:.005em}
.featured-pub-meta{font-size:.8rem;color:var(--text-light);font-weight:500;line-height:1.55;letter-spacing:.01em}
.featured-pub-link{font-family:var(--font-mono);font-size:.68rem;font-weight:700;letter-spacing:.1em;line-height:1.5;text-transform:uppercase;color:var(--primary-green);display:inline-flex;align-items:center;gap:.35rem;margin-top:.2rem;transition:gap .2s var(--transition-smooth)}
.featured-pub-card:hover .featured-pub-link{gap:.5rem;color:var(--dark-green)}
...
.tl-inner{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad);position:relative;z-index:2}
.tl-head{display:flex;flex-direction:column;align-items:center;text-align:center;gap:.6rem}
...
.tl-filters{display:flex;flex-wrap:wrap;justify-content:center;gap:.45rem;margin-top:.15rem}
...
.tl-stage{position:relative;z-index:2;margin-top:clamp(.6rem,1.3vw,.95rem)}
...
.tl-track{position:relative;display:flex;align-items:stretch;width:max-content;min-height:478px;padding:0 var(--shell-pad)}
.tl-item::before,.tl-year::before{content:'';position:absolute;left:0;right:0;top:50%;height:2px;margin-top:-1px;background:rgba(212,175,55,.82);box-shadow:0 0 10px rgba(212,175,55,.22);z-index:0;pointer-events:none}
...
.tl-year-pill{position:relative;z-index:1;display:inline-flex;align-items:center;gap:.4rem;font-family:var(--font-mono);font-weight:700;font-size:clamp(.92rem,1.9vw,1.12rem);letter-spacing:.13em;color:#0b1c16;background:linear-gradient(135deg,var(--gold-accent),#f0d478);padding:.4rem .8rem;border-radius:999px;box-shadow:0 12px 28px rgba(0,0,0,.4),0 0 0 4px rgba(11,28,22,.55);transition:transform .25s var(--transition-smooth),box-shadow .25s var(--transition-smooth)}
...
.tl-item{position:relative;flex:0 0 360px;z-index:1}
.tl-track.tl-anim .tl-card,.tl-track.tl-anim .tl-stem,.tl-track.tl-anim .tl-dot{opacity:0}
.tl-track.tl-anim .tl-item.is-in .tl-card,.tl-track.tl-anim .tl-item.is-in .tl-stem,.tl-track.tl-anim .tl-item.is-in .tl-dot{opacity:1}
.tl-dot{position:absolute;left:50%;top:50%;width:13px;height:13px;margin:-6.5px 0 0 -6.5px;border-radius:50%;background:var(--gold-accent);box-shadow:0 0 0 4px rgba(11,28,22,.95),0 0 16px rgba(212,175,55,.6);z-index:3;transition:transform .3s var(--transition-smooth),box-shadow .3s var(--transition-smooth),opacity .5s var(--transition-smooth)}
...
.tl-stem{position:absolute;left:50%;width:2px;margin-left:-1px;z-index:1;transition:opacity .5s var(--transition-smooth);background:linear-gradient(180deg,rgba(212,175,55,.6),rgba(212,175,55,.1))}
.tl-up .tl-stem{bottom:50%;height:40px}
.tl-down .tl-stem{top:50%;height:40px;background:linear-gradient(0deg,rgba(212,175,55,.6),rgba(212,175,55,.1))}
.tl-card{position:absolute;left:.6rem;right:.6rem;background:rgba(255,255,255,.055);border:1px solid rgba(232,245,240,.14);border-radius:16px;padding:.7rem .9rem .8rem;backdrop-filter:blur(10px);box-shadow:0 14px 36px rgba(0,0,0,.3);z-index:2;transition:transform .3s var(--transition-smooth),border-color .3s var(--transition-smooth),background .3s var(--transition-smooth),box-shadow .3s var(--transition-smooth),opacity .5s var(--transition-smooth)}
.tl-up .tl-card{bottom:calc(50% + 40px)}
.tl-down .tl-card{top:calc(50% + 40px)}
...
.tl-meta{display:flex;align-items:center;flex-wrap:wrap;gap:.4rem;margin-bottom:.35rem}
...
.tl-text{font-size:.93rem;line-height:1.45;color:rgba(255,255,255,.93);font-weight:500}
.tl-controls{display:flex;align-items:center;gap:1rem;margin-top:.6rem}
...
@media(max-width:860px){
  .tl-viewport{overflow-x:hidden;overflow-y:visible;cursor:auto;padding:0 var(--shell-pad)}
  .tl-track{flex-direction:column;width:100%;min-height:0;padding:0}
  .tl-item::before,.tl-year::before,.tl-track::before{display:none}
  .tl-year{width:100%;justify-content:flex-start;margin:.5rem 0 .35rem;padding:.3rem 0;cursor:pointer}
  ...
  .tl-group{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.5rem;overflow:hidden;max-height:0;opacity:0;transition:max-height .45s var(--transition-smooth),opacity .3s ease}
  .tl-group.is-open{max-height:3000px;opacity:1}
  .tl-item{flex:0 0 auto;width:100%;padding:0}
  .tl-track.tl-anim .tl-card,.tl-track.tl-anim .tl-stem,.tl-track.tl-anim .tl-dot{opacity:1}
  .tl-up .tl-card,.tl-down .tl-card{position:relative;top:auto;bottom:auto;left:0;right:0;height:100%;padding:.6rem .65rem .7rem;border-radius:12px}
  .tl-meta{gap:.3rem;margin-bottom:.28rem}
  .tl-month{font-size:.6rem;letter-spacing:.1em}
  .tl-kind{font-size:.52rem;padding:.1rem .38rem;letter-spacing:.06em}
  .tl-soon{font-size:.52rem;padding:.1rem .38rem}
  .tl-text{font-size:.81rem;line-height:1.38}
  .tl-stem,.tl-dot{display:none}
  .tl-controls{display:none}
  .tl-hint{display:none}
  .tl-hint-m{display:block}
}
/* ===== TEAM BAR ===== */
.hero-team-bar{position:relative;z-index:1;padding:clamp(.9rem,1.7vw,1.2rem) var(--shell-pad);background:rgba(9,24,19,.94);backdrop-filter:blur(12px);border-top:1px solid rgba(74,140,115,.22);border-bottom:1px solid rgba(74,140,115,.12)}
.hero-team-bar-inner{max-width:var(--shell-max);margin:0 auto;display:flex;flex-wrap:wrap;align-items:baseline;gap:.4rem 1rem;font-size:clamp(.8rem,1.6vw,.92rem);color:rgba(232,245,240,.78);font-family:var(--font-mono);line-height:1.65}
...
.section-container{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
.section-divider{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
...
.social-container{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
...
.footer-content{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
...
@media(max-width:768px){.page,#main,.initial-content,.page__inner-wrap,.page__content,.archive{width:100%!important}.lang-toggle{top:.9rem}.hero{padding:1.5rem 0 1.75rem}.hero-text{max-width:100%;gap:1.1rem}.animated-tagline{flex-direction:column;align-items:center;gap:.45rem}.tagline-pill{padding:.75rem 1.25rem .9rem;max-width:100%;border-radius:26px}.tagline-pill span#hero-tagline-static{font-size:clamp(1.6rem,6.4vw,2.1rem)}.hero-word{font-size:clamp(1.7rem,6.4vw,2.2rem)}.featured-pub-card{width:100%;align-items:center;text-align:center;padding:1rem 1.2rem 1.1rem}.featured-pub-card::before{width:100%;height:4px;top:0;left:0;bottom:auto;background:linear-gradient(90deg,var(--gold-accent),rgba(212,175,55,.25))}.image-gallery{grid-template-columns:repeat(2,1fr)}.gallery-item{grid-column:span 2;aspect-ratio:16/10;max-height:160px;box-shadow:var(--shadow-sm)}.buscadoras-image{max-width:100%;margin-top:1.2rem;box-shadow:var(--shadow-sm)}.collab-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:.6rem;margin-top:1.1rem}.collab-card{min-height:0;box-shadow:var(--shadow-sm);border-radius:14px}.collab-logo{min-height:58px;padding:.6rem .5rem .35rem}.collab-logo img{max-height:44px}.collab-card-gif .collab-logo{height:88px}.collab-meta{padding:.45rem .55rem .6rem}.collab-name{font-size:.76rem;line-height:1.3;letter-spacing:0}.collab-note{display:none}#collab-note-eaaf{display:block;font-size:.68rem;line-height:1.35;margin-top:.25rem}.social-grid{grid-template-columns:1fr}.social-embed{min-height:auto;box-shadow:var(--shadow-sm)}.iframe-container iframe{height:520px}.project-logo{width:80px}}
...
.gif-strip-inner{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad);position:relative;z-index:1;display:grid;grid-template-columns:1.55fr 1fr 1fr;gap:clamp(.55rem,1.2vw,.9rem);align-items:stretch}
...
.gif-strip-caption{position:relative;z-index:2;margin:clamp(.55rem,1.2vw,.85rem) 0 0;padding:clamp(.55rem,1.2vw,.85rem) var(--shell-pad);background:rgba(11,28,22,0.82);backdrop-filter:blur(12px);border-top:1px solid rgba(74,140,115,.22)}
.gif-caption-inner{max-width:var(--shell-max);margin:0 auto;display:flex;flex-wrap:wrap;align-items:baseline;gap:.55rem 1.1rem;font-size:clamp(.78rem,1.5vw,.88rem);color:rgba(232,245,240,.78);font-family:var(--font-mono);line-height:1.6}
...
.stats-ribbon-inner{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad);display:flex;justify-content:center;gap:clamp(2rem,5vw,5rem);flex-wrap:wrap}
...
.trailer-container{max-width:1000px;margin:0 auto;padding:0 var(--shell-pad);position:relative;z-index:1;text-align:center}
...
.copro-inner{max-width:var(--shell-max);margin:0 auto;padding:0 var(--shell-pad)}
...
/* ===== Tlajomulco example: clickable disclosure ===== */
.copro-ex-toggle{display:flex;align-items:flex-start;gap:.75rem;width:100%;text-align:left;background:none;border:none;padding:0;margin:0;font:inherit;color:inherit;cursor:pointer}
.copro-ex-heads{display:block;flex:1 1 auto;min-width:0}
.copro-ex-toggle .copro-example-label{display:block}
.copro-ex-toggle .copro-example-title{display:block;margin-bottom:0}
.copro-ex-chev{flex:0 0 auto;width:30px;height:30px;border-radius:50%;border:1px solid rgba(45,95,77,.22);background:rgba(232,245,240,.75);display:flex;align-items:center;justify-content:center;margin-top:.15rem;transition:transform .3s var(--transition-smooth),background .25s var(--transition-smooth),border-color .25s var(--transition-smooth)}
.copro-ex-chev::before{content:'\2922';font-size:.85rem;line-height:1;color:var(--primary-green)}
.copro-example:hover .copro-ex-chev{background:#fff;border-color:rgba(212,175,55,.6);transform:scale(1.08)}
/* ===== Four ways of seeing: card list for small screens ===== */
.fwos-cards{display:none;list-style:none;padding:0;margin:0;gap:.7rem}
.fwos-card{background:#fff;border:1px solid rgba(45,95,77,.13);border-left:3px solid var(--gold-accent);border-radius:14px;padding:.85rem .95rem;box-shadow:var(--shadow-sm)}
.fwos-card-k{display:block;font-family:var(--font-mono);font-size:.62rem;font-weight:700;letter-spacing:.11em;color:var(--primary-green);margin-bottom:.3rem}
.fwos-card-t{display:block;font-weight:750;font-size:1rem;line-height:1.3;color:var(--dark-green)}
.fwos-card-s{display:block;font-size:.87rem;line-height:1.5;color:var(--text-medium);margin-top:.2rem}
.fwos-card-core{background:linear-gradient(135deg,#0e2a1f,#0b1c16);border-color:transparent;border-left-color:var(--gold-accent)}
.fwos-card-core .fwos-card-t{color:#fff;font-family:var(--font-display);font-style:italic;font-weight:400;font-size:1.15rem}
.fwos-card-core .fwos-card-s{color:rgba(212,175,55,.9);font-family:var(--font-mono);font-size:.68rem;letter-spacing:.08em}
/* ===== Gallery ===== */
.gallery-hint{display:none;font-family:var(--font-mono);font-size:.7rem;letter-spacing:.04em;color:var(--text-light);margin-top:1.5rem}
@media(max-width:768px){
  /* Technologies: tighter list */
  #technologies{padding:1.15rem 1rem 1.35rem;border-radius:20px}
  .info-list{gap:.5rem;margin-top:.9rem}
  .info-list li{padding:.6rem .7rem .6rem 2.2rem;min-height:0;font-size:.92rem;line-height:1.4;border-radius:12px}
  .info-list li::before{width:23px;height:23px;font-size:.85rem;left:.6rem}
  /* Gallery: one swipeable strip instead of six stacked rows */
  .gallery-hint{display:block;margin-top:1.25rem;margin-bottom:.5rem}
  .image-gallery{display:flex;gap:.6rem;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;margin-top:0;padding-bottom:.35rem}
  .image-gallery::-webkit-scrollbar{display:none}
  .gallery-item{flex:0 0 74%;max-width:74%;grid-column:auto;aspect-ratio:4/3;max-height:none;scroll-snap-align:center;border-radius:16px}
  .gallery-item::before{content:'\2922';position:absolute;right:.45rem;bottom:.45rem;z-index:2;width:27px;height:27px;display:flex;align-items:center;justify-content:center;font-size:.9rem;color:#fff;background:rgba(11,28,22,.6);border:1px solid rgba(255,255,255,.32);border-radius:9px;pointer-events:none}
  .touch-zoomable.is-expanded::before{content:'\00d7';position:fixed;top:1rem;right:1rem;left:auto;bottom:auto;width:44px;height:44px;font-size:1.7rem;border-radius:50%;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.4);z-index:10000}
  /* Coproduction reads better as one column with a tighter quote */
  .copro-quote{padding:1.25rem 1.15rem 1.35rem;border-radius:var(--radius-md)}
  .copro-quote::before{font-size:5rem;top:-.28em}
  .copro-quote-text{font-size:1.12rem;line-height:1.45;margin-bottom:.75rem}
  .copro-example{padding:1.1rem 1rem 1.2rem}
  .copro-grid{gap:1rem;margin-top:1.25rem}
  .copro-lead{font-size:1.15rem}
  .copro-body{font-size:1rem;line-height:1.7}
  /* Four ways: swap the wide diagram for the cards */
  .fwos-figure{display:none}
  .fwos-cards{display:grid}
  .fwos{margin-top:1.75rem}
  .fwos-sub{font-size:.98rem;line-height:1.65;margin-bottom:1rem}
  .fwos-caption{margin-top:.9rem;font-size:.72rem}
}

/* ===== Tlajomulco: preview card opens a square modal ===== */
.copro-ex-toggle{cursor:pointer}
.copro-example{cursor:pointer}
.copro-example:hover{border-color:rgba(212,175,55,.5);box-shadow:var(--shadow-lg)}
.copro-ex-preview{position:relative}
.copro-ex-preview .copro-example-text{display:-webkit-box;-webkit-line-clamp:3;line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
.copro-modal{position:fixed;inset:0;z-index:9998;display:flex;align-items:center;justify-content:center;padding:clamp(1rem,4vw,2rem);background:rgba(6,18,14,.62);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);opacity:0;transition:opacity .28s var(--transition-smooth)}
.copro-modal[hidden]{display:none}
.copro-modal.is-open{opacity:1}
.copro-modal-panel{position:relative;width:min(92vw,600px);aspect-ratio:1/1;height:auto;max-height:88vh;overflow-y:auto;background:#fff;border-radius:var(--radius-lg);border:1px solid rgba(45,95,77,.14);box-shadow:0 30px 70px rgba(0,0,0,.35);padding:clamp(1.5rem,3.5vw,2.5rem);transform:translateY(14px) scale(.97);transition:transform .3s var(--transition-smooth)}
.copro-modal.is-open .copro-modal-panel{transform:none}
.copro-modal-panel::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;border-radius:var(--radius-lg) var(--radius-lg) 0 0;background:linear-gradient(90deg,var(--gold-accent),rgba(212,175,55,.25))}
.copro-modal-close{position:absolute;top:.75rem;right:.75rem;width:38px;height:38px;border-radius:50%;border:1px solid rgba(45,95,77,.18);background:rgba(232,245,240,.7);color:var(--dark-green);font-size:1.4rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s ease,transform .2s ease}
.copro-modal-close:hover{background:#fff;transform:scale(1.06)}
.copro-modal-label{font-family:var(--font-mono);font-size:.7rem;font-weight:750;letter-spacing:.12em;text-transform:uppercase;color:var(--light-green);margin-bottom:.55rem;padding-right:2.5rem}
.copro-modal-title{font-size:clamp(1.15rem,2.4vw,1.5rem);font-weight:800;color:var(--dark-green);line-height:1.3;letter-spacing:-.01em;margin:0 0 .9rem;padding-bottom:0}
.copro-modal-title::after{display:none}
.copro-modal-text{font-size:clamp(.98rem,2vw,1.05rem);line-height:1.8;color:var(--text-medium)}
.copro-modal-text strong{color:var(--primary-green);font-weight:700}
body.copro-modal-open{overflow:hidden}
@media(max-width:520px){.copro-modal-panel{aspect-ratio:auto}}
</style>
<!-- TITLE SECTION -->
  ...
  <div class="hero-content">
    <div class="hero-text reveal">
      <div class="animated-tagline">
        ...
        <a class="featured-pub-card reveal reveal-delay-2" href="https://www.frontiertechhub.org/insights/ethics-for-the-disappeared-in-a-technological-age" target="_blank" rel="noopener noreferrer" aria-label="Featured publication: An ethics for the disappeared in a technological age">
          <span class="featured-pub-eyebrow"><span aria-hidden="true">&#x2726;</span><span id="featured-pub-eyebrow">Featured publication</span></span>
          <span class="featured-pub-title" id="featured-pub-title">An ethics for the disappeared in a technological age</span>
          <span class="featured-pub-meta">Miguel Moctezuma, Daniele Rugo &amp; Brad Evans (2026)</span>
          <span class="featured-pub-link" id="featured-pub-link">Read the article <span aria-hidden="true">&#x2197;</span></span>
        </a>
      </div>
...
<section class="stats-ribbon reveal" aria-label="Key statistics">
  <div class="stats-ribbon-inner">
    ...
    <div class="stat-divider"></div>
    <div class="stat-item"><div class="stat-number"><span class="stat-value" data-to="2">2</span></div><div class="stat-label" id="stat-label-3">Countries</div></div>
    <div class="stat-divider"></div>
    <div class="stat-item"><div class="stat-number"><span class="stat-value" data-to="5">5</span></div><div class="stat-label" id="stat-label-4">Experimental sites</div></div>
  </div>
</section>
<!-- ===== DOCUMENTARY TRAILER ===== -->
  ...
  <div class="section-container dual-sections-grid">
    <div class="dual-column reveal">
      <section id="technologies">
        ...
        <p class="gallery-hint" id="gallery-hint">Tap to enlarge &middot; swipe for more</p>
  ...
  <div class="copro-inner">
    ...
    <div class="copro-grid">
      <figure class="copro-quote">
        <p class="copro-quote-text" id="copro-quote-text">When families speak, they speak in the present tense. "He is a son." "She is a student." The verb stays in present tense. There is an ethic in that grammar.</p>
    ...
    <div class="fwos">
      ...
      <ul class="fwos-cards">
        <li class="fwos-card"><span class="fwos-card-k" id="fwosm-n1-k"></span><span class="fwos-card-t" id="fwosm-n1-t"></span><span class="fwos-card-s" id="fwosm-n1-s"></span></li>
        <li class="fwos-card"><span class="fwos-card-k" id="fwosm-n2-k"></span><span class="fwos-card-t" id="fwosm-n2-t"></span><span class="fwos-card-s" id="fwosm-n2-s"></span></li>
        <li class="fwos-card"><span class="fwos-card-k" id="fwosm-n3-k"></span><span class="fwos-card-t" id="fwosm-n3-t"></span><span class="fwos-card-s" id="fwosm-n3-s"></span></li>
        <li class="fwos-card"><span class="fwos-card-k" id="fwosm-n4-k"></span><span class="fwos-card-t" id="fwosm-n4-t"></span><span class="fwos-card-s" id="fwosm-n4-s"></span></li>
        <li class="fwos-card fwos-card-core"><span class="fwos-card-t" id="fwosm-core"></span><span class="fwos-card-s" id="fwosm-coresub"></span></li>
      </ul>
...
<div class="copro-modal" id="copro-modal" role="dialog" aria-modal="true" aria-labelledby="copro-modal-title" hidden>
  <div class="copro-modal-panel" role="document">
    <button type="button" class="copro-modal-close" id="copro-modal-close" aria-label="Close">&times;</button>
    <div class="copro-modal-label" id="copro-modal-label"></div>
    <h3 class="copro-modal-title" id="copro-modal-title"></h3>
    <div class="copro-modal-text" id="copro-modal-text"></div>
  </div>
</div>
...
(function(){
  ...
  function setLanguage(lang){
    ...
    buildHeroWords(lang);startWordRotation();tlRender(lang);syncFwosCards();
  }
  /* the small-screen card list mirrors the diagram, so it never drifts out of sync */
  function syncFwosCards(){
    [1,2,3,4].forEach(function(i){
      ['k','t','s'].forEach(function(sfx){
        var src=document.getElementById('fwos-n'+i+'-'+sfx),dst=document.getElementById('fwosm-n'+i+'-'+sfx);
        if(src&&dst)dst.textContent=src.textContent;
      });
    });
    var c1=document.getElementById('fwos-core1'),c2=document.getElementById('fwos-core2'),
        s1=document.getElementById('fwos-sub1'),s2=document.getElementById('fwos-sub2'),
        dc=document.getElementById('fwosm-core'),ds=document.getElementById('fwosm-coresub');
    if(dc&&c1&&c2)dc.textContent=(c1.textContent+' '+c2.textContent).trim();
    if(ds&&s1&&s2)ds.textContent=(s1.textContent+' '+s2.textContent).replace(/\s+/g,' ').trim();
  }
  function setupCoproModal(){
    var card=document.querySelector('.copro-example'),modal=document.getElementById('copro-modal'),
        closeBtn=document.getElementById('copro-modal-close');
    if(!card||!modal)return;
    var lastFocus=null;
    function fill(){
      var src={label:'copro-ex-label',title:'copro-ex-title',text:'copro-ex-text'},
          dst={label:'copro-modal-label',title:'copro-modal-title',text:'copro-modal-text'};
      Object.keys(src).forEach(function(k){
        var a=document.getElementById(src[k]),b=document.getElementById(dst[k]);
    ...
    function open(){
      ...
      modal.hidden=false;
      document.body.classList.add('copro-modal-open');
      requestAnimationFrame(function(){modal.classList.add('is-open')});
      if(closeBtn)closeBtn.focus();
    }
    function close(){
      modal.classList.remove('is-open');
      document.body.classList.remove('copro-modal-open');
      setTimeout(function(){modal.hidden=true},reduceMotion?0:280);
  ...
  function init(){
    ...
    setupTouchZoom();setupScrollReveal();setupCounters();setupCoproModal();
  }
